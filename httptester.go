// Package httptester implements functions for testing and benchmarking
// HTTP server based applications.
//
...
// -httptester.host command line flag. e.g.
//
//  go test -v -httptester.host example.com // or https://example.com
//
	...
	if r.resp == nil {
		if r.err == nil {
			...
			if err != nil {
				...
				r.Tester.TB.Log(fmt.Sprintf("requesting %s", req.URL))
				start := time.Now()
				if remoteHost != "" {
					client := &http.Client{}
					...
					r.resp = new(response)
					r.Tester.Handler.ServeHTTP(r.resp, req)
				}
				r.Tester.TB.Log(fmt.Sprintf("received response (%d bytes) with code %d in %s", r.resp.body.Len(), r.resp.code, time.Since(start)))
				r.setErr(r.resp.err)
			}
		}
	}
...
// so it ignores the -httptester.host flag.
func (r *Request) Bench(b *testing.B) {
	req, err := r.asHTTPRequest()
	if err != nil {
		r.Tester.TB.Fatal(err)
		return
	}
	...
	// Do the request once, to allow any on-demand data
	// to be loaded
...
func (r *Request) AddHeader(key string, value interface{}) *Request {
	...
	r.Header.Add(key, toString(value))
	return r
}
...
func (r *Request) expect(what interface{}, name string, value interface{}) *Request {
	if r.do() {
		...
		default:
			r.Tester.TB.Fatal(fmt.Errorf("don't know what to expect from %T", what))
		}
	}
	return r
}
...
// its own logging.
func New(tb testing.TB, handler http.Handler) *Tester {
	if remoteHost != "" {
		tb.Log(fmt.Sprintf("using host %s", remoteHost))
	}
	return &Tester{tb, handler}
}
...
func init() {
	if insideGoTest() {
		flag.StringVar(&remoteHost, "httptester.host", "", "Host to run the tests against")
		// Note there's no need to parse the flags because go test will call flag.Parse() for us
	}
}
